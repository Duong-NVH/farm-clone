...
interface IMigratorChef {
    ...
    function migrate(IFRC20 token) external returns (IFRC20);
}
...
contract MasterChef is Ownable {
    ...
    // Info of each pool.
    struct PoolInfo {
        IFRC20 lpToken; // Address of LP token contract.
    ...
    function migrate(uint256 _pid) public {
        ...
        IFRC20 lpToken = pool.lpToken;
        uint256 bal = lpToken.balanceOf(address(this));
        lpToken.safeApprove(address(migrator), bal);
        IFRC20 newLpToken = migrator.migrate(lpToken);
